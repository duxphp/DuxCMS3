...
class WechatMobilePay extends \app\base\service\BaseService {
    ...
    public function getConfig() {
        ...
        $notifyUrl = url('api/wechat/WechatMobile/index',[], true);
        return [
    ...
    public function getData($data) {
        ...
        $typeConfig = target('member/PayConfig')->typeInfo($this->name);
        $oathUser = target('member/MemberConnect')->getWhereInfo([
            'user_id' => $data['user_id'],
            'type' => $typeConfig['oauth']
        ]);
        if(empty($oathUser)) {
            return $this->error('该用户未绑定微信');
        }
        $payData['openid'] = $oathUser['open_id'];
